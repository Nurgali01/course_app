...
import 'package:course_app/src/on_boarding/domain/usecases/cache_first_timer.dart';
import 'package:course_app/src/on_boarding/domain/usecases/check_if_user_is_first_timer.dart';
...
void main() {
  ...
  final tFailure = CacheFailure(message: 'Test Failure', statusCode: 404);
  ...
  group('cacheFirstTimer', () {
    blocTest<OnBoardingCubit, OnBoardingState>(
      ...
      build: () {
        ...
        return cubit; // Fixed variable name
      },
      act: (cubit) => cubit.cacheFirstTimer(),
      expect: () => [
        CachingFirstTimer(),
        UserCached(), // Emit UserCached when successful
      ],
      verify: (_) {
    ...
    blocTest<OnBoardingCubit, OnBoardingState> (
      'should emit '
          '[CachingFirstTimer, OnBoardingError] when unsuccessful', build: () {
      when(() => cacheFirstTimer()).thenAnswer(
  ...
  group('checkIfUserIsFirstTimer', () {
    blocTest<OnBoardingCubit, OnBoardingState>(
      'should emit [CheckingIfUserIsTimer, OnBoardingStatus] '
          'when successful',
      build: () {
        when(() => checkIfUserIsFirstTimer()).thenAnswer((_) async =>const Right(false),);
        return cubit;
      },
    ...
    blocTest<OnBoardingCubit, OnBoardingState>(
      'should emit [CheckingIfUserIsTimer, OnBoardingError] when '
          'unsuccessful',
      build: () {
